...
if (theme_get_setting('sigmah_theme_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('sigmah_theme_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'sigmah_theme') .'/css/tabs.css', 'theme', 'screen');
}
...
function sigmah_theme_preprocess_page(&$vars, $hook) {
	//echo print_r(debug_backtrace(), 1);
  if($vars['is_front']){
	drupal_add_js(drupal_get_path('theme', 'sigmah_theme') . '/js/script.js','theme');
	drupal_add_js(drupal_get_path('theme', 'sigmah_theme') . '/fancybox/jquery.mousewheel-3.0.4.pack.js','theme');
	drupal_add_js(drupal_get_path('theme', 'sigmah_theme') . '/fancybox/jquery.fancybox-1.3.4.pack.js','theme');
  }else{
	drupal_add_js(drupal_get_path('theme', 'sigmah_theme') . '/js/script_page.js','theme');
  }
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('sigmah_theme_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = sigmah_theme_id_safe('page-'. $path);
    $body_classes[] = sigmah_theme_id_safe('section-'. $section);
  ...
  $vars['head_title'] = drupal_set_title($title.$title_separator.$page_title);      
...
function sigmah_theme_preprocess_node(&$vars, $hook) {
	//kpr($vars);
	$vars['content'] = str_replace("<blockquote>", '<div class="big_miseenavant">
            <div class="topmise"></div>
            <div class="midlemise">', $vars['content']);
	$vars['content'] = str_replace("</blockquote>", ' </div>
            <div class="bottommise"></div>
            </div>', $vars['content']);	
	$vars['body'] = str_replace("<blockquote>", '<div class="big_miseenavant">
            <div class="topmise"></div>
            <div class="midlemise">', $vars['content']);
	$vars['body'] = str_replace("</blockquote>", ' </div>
            <div class="bottommise"></div>
            </div>', $vars['content']);	
	$vars['preview'] = str_replace("<blockquote>", '<div class="big_miseenavant">
            <div class="topmise"></div>
            <div class="midlemise">', $vars['content']);
	$vars['preview'] = str_replace("</blockquote>", ' </div>
            <div class="bottommise"></div>
            </div>', $vars['content']);				
  ...
  $classes[] = sigmah_theme_id_safe('node-type-' . $vars['type']);
...
function sigmah_theme_preprocess_block(&$vars, $hook) {
	
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = sigmah_theme_id_safe('block-' . $vars['block']->module);
    $classes[] = sigmah_theme_id_safe('block-' . $vars['block']->region);
    $classes[] = sigmah_theme_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('sigmah_theme_block_editing') && user_access('administer blocks')) {
		...
		if(arg(0) == 'contact'){
			$vars['is_contact'] = true;
		}else{
			$vars['is_contact'] = false;
		}
      }
  }
...
function sigmah_theme_preprocess_comment(&$vars, $hook) {
...
function sigmah_theme_menu_item_link($link) {
...
function sigmah_theme_menu_local_tasks() {
  $output = '';
...
function sigmah_theme_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = sigmah_theme_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function sigmah_theme_id_safe($string) {
...
function sigmah_theme_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb">'. implode(' » ', $breadcrumb) .'</div>';
  }
}

function sigmah_theme_fieldgroup_fieldset($element){
	return '';
}
