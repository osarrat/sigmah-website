	...
	if ( $t_user_id == 0 ) {
		...
		if ( $t_map === false || ( count( $t_map ) == 0 ) ) {
			...
			if ( isset( $t_map_name_as_key[$t_username] ) ) {
				...
				#update t_username with existing value for logging in
				$t_username = $t_map[$t_user_id];
			}
		...
		# If a user id is found/created and the rpxnow account is advanced, then update rpxnow.	
		if ( $t_user_id != 0 && config_get( 'openid_rpxnow_advanced_account' ) ) {
			try
			{
